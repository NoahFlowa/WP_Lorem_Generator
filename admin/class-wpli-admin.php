...
if ( ! class_exists( 'WPLoremIpsum_Admin' ) ) :
    ...
    class WPFaker_Admin {
        ...
        public function custom_admin_menu() {
            ...
            add_options_page(
                __('WP Faker Settings', 'wp-faker'),
                __('WP Faker', 'wp-faker'),
                'manage_options',
                'wp-faker',
            ...
            <div class="wrap">
                <?php if( isset($_GET['message']) && $_GET['message'] == 'success' ) : ?>
                    <div class="notice notice-success is-dismissible">
                        <p><?php _e('Operation performed successfully!', 'wp-faker'); ?></p>
                    </div>
                <?php endif; ?>
                <h2><?php _e('WP Faker', 'wp-faker'); ?></h2>
                <p><?php _e('You can create fake posts to fill your database.', 'wp-faker'); ?></p>
                <form method="post" action="<?php echo admin_url('admin-post.php'); ?>" class="li-form">
                    <table width="100%" cellspacing="0" cellpadding="0">
                        <tbody>
                            <tr>
                                <td><label for="post-count"><?php _e('Number of posts', 'wp-faker'); ?></label></td>
                                <td>
                                    <select name="post_count" id="post-count" required="required">
                                        <option value=""><?php _e('Select', 'wp-faker'); ?>...</option>
                                        <?php for ( $i = 1; $i <= 1; $i++ ) : ?>
                            ...
                            <tr>
                                <td><label for="post-type"><?php _e('Post type', 'wp-faker'); ?></label></td>
                                <td>
                            ...
                            <tr>
                                <td><label for="paras"><?php _e('Paragraphs', 'wp-faker'); ?></label></td>
                                <td>
                                    <select name="paras" id="paras" required="required">
                                        <option value=""><?php _e('Select', 'wp-faker'); ?>...</option>
                                        <option value="0"><?php _e('Random', 'wp-faker'); ?></option>
                                        <?php for ( $i = 1; $i <= 1; $i++ ) : ?>
                                            <option value="<?php echo $i; ?>"><?php echo $i; ?></option>
                                        <?php endfor; ?>
                                    </select>
                            ...
                            <tr>
                                <td><label for="post-status"><?php _e('Post status', 'wp-faker'); ?></label></td>
                                <td>
                            ...
                            <tr>
                                <td><label for="post-author"><?php _e('Post author', 'wp-faker'); ?></label></td>
                                <td>
                            ...
                            <tr>
                                <td><label for="has-post-thumbnail"><?php _e('Post thumbnail', 'wp-faker'); ?></label></td>
                            </tr>
                        </tbody>
                    </table>
                    <button type="submit" class="button button-primary button-large"><?php  _e('Send', 'wp-faker'); ?></button>
        ...
        public function post_submit_save() {
            ...
            if ( isset( $_POST['_wpnonce'] ) && wp_verify_nonce( $_POST['_wpnonce'], get_bloginfo('name') ) ) :
                ...
                $redirect_ok = add_query_arg( 'page', 'wp-faker', $redirect_ok );
                ...
                for ( $i = 1; $i <= 1; $i++ ) :
                    ...
                    $post_content = "Address: $faker->address\nEmail: $faker->email\nPhone Number: $faker->phoneNumber\nCatch Phrase: $faker->catchPhrase\n";
                ...
                // Error during insert
                if( wp_die( __('Error.', 'wp-faker') ) ) exit;
        ...
        /**
         * Generate a preformatted paragraph with Faker
         */
    ...
    add_action( 'plugins_loaded', [ 'WPFaker_Admin', 'init' ] );
